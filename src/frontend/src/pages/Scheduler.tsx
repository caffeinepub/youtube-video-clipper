...
import type { ScheduledUpload, VideoClip } from "../backend";
...
function useAddScheduledUpload() {
  ...
  return useMutation({
    mutationFn: async ({
      clipId,
      scheduledAt,
    }: { clipId: string; scheduledAt: bigint }) => {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Schedule your clips for YouTube upload
          </p>
        </div>
      </div>
      ...
      <div className="glass-card p-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground text-xs">
            {isConnected
              ? channelStatus?.channelName || "Connected"
              : "Not connected"}
          </p>
        </div>
        {isConnected ? (
      ...
      <div className="space-y-3">
        ...
        {uploadsLoading ? (
          <div className="space-y-2">
            {["s1", "s2", "s3"].map((k) => (
              <Skeleton key={k} className="h-16 bg-white/5 rounded-xl" />
            ))}
          </div>
        ) : !uploads || uploads.length === 0 ? (
                  ...
                  <AlertDialogContent className="bg-[#0B0E14] border-white/10">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="text-muted-foreground">
                        This will cancel the scheduled upload for "
                        {getClipTitle(upload.clipId)}".
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
