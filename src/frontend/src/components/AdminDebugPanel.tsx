...
export default function AdminDebugPanel() {
    ...
    return (
      <Card className="border-2 border-primary/20 bg-primary/5">
        <CardHeader>
          ...
          <CardDescription>
            Loading authentication debug information...
          </CardDescription>
        </CardHeader>
  ...
  const backendMethodMissing =
    adminPrincipals.length === 0 &&
    hashMapSize === 0 &&
    initStatus.includes("not yet implemented");
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Comprehensive authentication and authorization debugging
            </CardDescription>
          </div>
          <Button onClick={handleRefresh} variant="outline" size="sm">
            <RefreshCw className="w-4 h-4 mr-2" />
      ...
      <CardContent className="space-y-6">
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription className="text-xs mt-2 space-y-2">
              <p className="font-semibold">
                The backend does not yet have the{" "}
                <code className="font-mono bg-background/50 px-1 rounded">
                  getAdminDebugInfo()
                </code>{" "}
                method.
              </p>
              ...
              <p className="mt-2">
                Currently showing limited debug information using{" "}
                <code className="font-mono bg-background/50 px-1 rounded">
                  isCallerAdmin()
                </code>{" "}
                only.
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-foreground">
              Registered Admin Principals{" "}
              {adminPrincipals.length > 0 && `(${adminPrincipals.length})`}
            </h3>
            {adminPrincipals.length > 0 &&
              hashMapSize !== adminPrincipals.length && (
          ...
          {adminPrincipals.length > 0 ? (
            <div className="space-y-3">
              {adminPrincipals.map((adminPrincipal, index) => {
                ...
                return (
                  <div key={`admin-${adminPrincipal}`} className="space-y-2">
                    <div
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          onClick={() =>
                            copyToClipboard(adminPrincipal, `admin-${index}`)
                          }
                          variant="ghost"
                          size="sm"
                          className="shrink-0"
                    ...
                    {!isMatch && comparison.differences.length > 0 && (
                      ...
                      >
                        ...
                        <AlertTitle className="text-xs font-semibold">
                          {comparison.differences.length} Character Difference
                        ...
                        <AlertDescription className="text-xs space-y-2 mt-2">
                          {comparison.differences.length < 20 ? (
                            <>
                              <div className="space-y-1">
                                {comparison.differences
                                  ...
                                  .map((diff, i) => (
                                    <div
                                      key={`diff-${i}-${diff.index}`}
                                      className="font-mono bg-background/50 p-1 rounded"
                                    >
                              ...
                              {comparison.differences.length > 10 && (
                                <div className="text-muted-foreground italic">
                                  ...and {comparison.differences.length - 10}{" "}
                                  more differences
                                </div>
                              )}
                            </>
                            ...
                            <div>
                              <p className="font-semibold">
                                Principals are completely different
                              </p>
                              <p className="mt-1">
                                Your principal: {currentPrincipal.length} chars
                                <br />
              ...
              <AlertDescription className="text-xs mt-2 space-y-2">
                {backendMethodMissing ? (
                  <>
                    ...
                    <p className="mt-2">
                      <strong>Current Status:</strong> Can only check if you're
                      an admin, but cannot see who the admins are.
                    </p>
                  </>
                ) : (
                  <>
                    <p>
                      The backend returned an empty list of admin principals.
                    </p>
                    <p className="font-semibold">
                    ...
                    <p className="mt-2">
                      <strong>HashMap Size:</strong> {hashMapSize} (should be at
                      least 1)
                    </p>
                  </>
                )}
        ...
        <Alert>
          ...
          <AlertDescription className="text-xs space-y-2 mt-2">
            <ol className="list-decimal list-inside space-y-1">
              ...
              <li>
                Copy your principal ID and compare it manually with expected
                admin principal
              </li>
              <li>Try logging out and logging back in</li>
              <li>
                Check if the admin token was correctly passed during deployment
              </li>
              <li>
                Verify the backend initialization logs in the canister logs
              </li>
            </ol>
            <div className="mt-3 p-2 bg-background rounded border">
              ...
              <ul className="space-y-1">
                ...
                <li>
                  ✓ Backend Method:{" "}
                  {backendMethodMissing ? "❌ Not Implemented" : "✅ Available"}
                </li>
                <li>
                  ✓ Principal Length: {currentPrincipal.length} characters
                </li>
              </ul>
            </div>
          </AlertDescription>
