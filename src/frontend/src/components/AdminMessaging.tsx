...
interface AdminMessagingWithPrefillProps {
  prefillPrincipal?: string;
  onSuccess?: () => void;
}

export function AdminMessagingWithPrefill({
  prefillPrincipal = "",
  onSuccess,
}: AdminMessagingWithPrefillProps) {
  const { actor } = useActor();
  const { identity } = useInternetIdentity();
  const queryClient = useQueryClient();

  const [toPrincipal, setToPrincipal] = useState(prefillPrincipal);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <Button
          ...
          disabled={
            sendMutation.isPending || !toPrincipal.trim() || !body.trim()
          }
