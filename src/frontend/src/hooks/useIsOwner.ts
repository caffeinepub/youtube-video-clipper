...
export function useIsOwner() {
  ...
  const query = useQuery<boolean>({
    ...
    queryFn: async () => {
      ...
      if (!actor) {
        console.error('[useIsOwner] Actor not available - this should not happen due to enabled condition');
        console.groupEnd();
        throw new Error('Actor not available');
      }
      
      const principalId = identity?.getPrincipal().toString() || 'unknown';
      console.log('[useIsOwner] Calling backend isCallerAdmin() for principal:', principalId);
      ...
      try {
        ...
        try {
          ...
          if (profile && profile.name) {
            ...
            if (parts.length === 2) {
              console.log('[useIsOwner] Extracted email from profile:', parts[1]);
            }
          }
        } catch (profileError) {
          console.log('[useIsOwner] Could not fetch profile (may not exist yet):', profileError);
        }
  ...
  useEffect(() => {
    ...
    console.groupEnd();
  }, [actor, actorFetching, identity, query.isLoading, query.isFetching, query.isFetched, query.isSuccess, query.isError, query.data, query.error]);
