...
export default function VerticalClipPreview({ clip }: VerticalClipPreviewProps) {
  if (!clip) {
    return (
      <div className="glass-card flex flex-col items-center justify-center p-4" style={{ aspectRatio: '9/16', maxHeight: '480px' }}>
        <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mb-4">
          <Film className="w-8 h-8 text-muted-foreground" />
        </div>
        <p className="text-muted-foreground text-sm font-medium text-center">No clip selected</p>
        <p className="text-muted-foreground/60 text-xs text-center mt-1">Select a clip to preview</p>
      </div>
    );
  }
  ...
  const videoIdMatch = clip.videoUrl.match(/(?:v=|youtu\.be\/|embed\/)([^&?/]+)/);
  ...
  return (
    <div className="glass-card overflow-hidden" style={{ aspectRatio: '9/16', maxHeight: '480px' }}>
      {/* Thumbnail / Preview */}
      <div className="relative w-full h-full flex flex-col">
        ...
        <div className="p-3 bg-gradient-to-t from-black/80 to-transparent absolute bottom-0 left-0 right-0">
          <h3 className="text-white font-semibold text-sm line-clamp-2 mb-1">{clip.title}</h3>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="font-mono text-indigo-400">{formatTime(startTime)}</span>
            <span>→</span>
            <span className="font-mono text-indigo-400">{formatTime(endTime)}</span>
          </div>
        </div>
      </div>
    </div>
