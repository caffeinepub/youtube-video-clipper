...
import { Activity, TrendingUp, MousePointerClick, Video, Loader2 } from 'lucide-react';
  ...
  if (active && payload && payload.length) {
    return (
      <div className="glass-card px-3 py-2 text-xs">
        ...
        <p className="text-indigo-400 font-semibold">{payload[0].value} videos</p>
      </div>
    );
  }
  return null;
...
export default function AppAnalytics() {
  const { totalClips, trendingAnalytics, isLoading: statsLoading } = useAdminStats();
    ...
    <div className="space-y-4">
          ...
          return (
            <div key={card.label} className="glass-card p-4">
              ...
              {statsLoading || clickLoading ? (
                ...
                <p className="text-white font-bold text-2xl font-display">{card.value}</p>
              )}
            </div>
          );
        })}
      ...
      <div className="glass-card p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-muted-foreground text-xs ml-auto">Last 24h</span>
        </div>
        ...
        {chartLoading ? (
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={videosPerHour} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="count" fill="oklch(0.55 0.22 264)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
