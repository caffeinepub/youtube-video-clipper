...
import { TrendingUp, Flame, BarChart2 } from 'lucide-react';
          ...
          {trendingAnalytics.slice(0, 5).map((clip, index) => (
            <div
              ...
              className={`flex items-center gap-3 p-2.5 rounded-lg transition-all hover:bg-white/5 ${
                index === 0 ? 'bg-indigo-500/10 border border-indigo-500/20' : ''
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  ...
                  <span className="text-orange-400 text-xs">{clip.trendingScore.toFixed(0)}</span>
                </div>
              </div>
            </div>
          ))}
