...
import { Plus, Edit2, Trash2, FileText, Loader2, Lock } from 'lucide-react';
...
import { ContentEntry } from '../backend';
...
export default function ContentManager() {
  ...
  const [formData, setFormData] = useState<EntryFormData>({ title: '', body: '' });
  ...
  const handleCreate = async () => {
    ...
    await createMutation.mutateAsync({ title: formData.title, body: formData.body });
  ...
  const handleUpdate = async () => {
    ...
    await updateMutation.mutateAsync({ id: editEntry.id, title: formData.title, body: formData.body });
    setEditEntry(null);
    setFormData({ title: '', body: '' });
  };
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-white font-bold text-2xl font-display">Content Manager</h1>
            <p className="text-muted-foreground text-sm">Manage content entries</p>
          </div>
        </div>
        <Button
          onClick={() => { setFormData({ title: '', body: '' }); setShowCreateDialog(true); }}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center glass-card">
          ...
          <h3 className="text-white font-semibold text-lg mb-2">No Content Entries</h3>
          <p className="text-muted-foreground text-sm mb-4">Create your first content entry to get started.</p>
          <Button
            onClick={() => { setFormData({ title: '', body: '' }); setShowCreateDialog(true); }}
            className="bg-indigo-500 hover:bg-indigo-600 text-white gap-2"
          >
        ...
        <div className="space-y-3">
          {entries.map((entry) => (
            <div key={entry.id} className="glass-card p-4 hover:bg-white/8 transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-white font-semibold text-sm mb-1">{entry.title}</h3>
                  <p className="text-muted-foreground text-xs line-clamp-2">{entry.body}</p>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="bg-[#0B0E14] border-white/10 text-white">
          <DialogHeader>
            <DialogTitle className="text-white">Create Content Entry</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <Input
      ...
      <Dialog open={!!editEntry} onOpenChange={(open) => { if (!open) setEditEntry(null); }}>
        <DialogContent className="bg-[#0B0E14] border-white/10 text-white">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setEditEntry(null)} className="text-muted-foreground">
              Cancel
            </Button>
            <Button
