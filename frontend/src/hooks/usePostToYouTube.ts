...
export function usePostToYouTube() {
  ...
  return useMutation({
    mutationFn: async (clipMetadata: ClipMetadata) => {
      ...
      if (!isConnected) {
        throw new Error('YouTube channel not connected. Please connect your channel first.');
      }

      // Race the actual post against a timeout
      const postPromise = actor.postClipToYouTube(clipMetadata);
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('YouTube post timed out after 30 seconds. Please try again.')), YOUTUBE_POST_TIMEOUT_MS)
      );
    ...
    onSuccess: (result) => {
      toast.success('Clip posted to YouTube!', {
        description: result.videoUrl ? `View at: ${result.videoUrl}` : undefined,
      });
